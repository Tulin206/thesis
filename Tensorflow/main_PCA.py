...
resnet18_PCA.model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001),
...
for i, (train_index, val_index) in enumerate(Skfold.split(Train_data, Label_Data)):
    ...
    history = resnet18_PCA.model.fit(train_aug,
                                 # X_train, y_train,
                                 batch_size=1,
                                 # steps_per_epoch=10,
                                 epochs=100,
    ...
    train_accuracy = resnet18_PCA.model.evaluate(train_aug, verbose=1)
    print('Train Loss, AUC and Training Accuracy', train_accuracy)
    val_accuracy = resnet18_PCA.model.evaluate(X_val, y_val, verbose=1)
    ...
    y_pred_proba = resnet18_PCA.model.predict(X_val)
...
preds = resnet18_PCA.model.predict(y)
...
y_pred_proba = resnet18_PCA.model.predict(Train_data)
