...
from TrainingData_SR_PyTorch import train, validate
from DataLoad_SR_PyTorch import save_plots, get_data, predict_test_data
...
model = LogisticRegression(49152, 1).to(device)
print(model)
...
if __name__ == '__main__':
    ...
    for fold, (train_loader, valid_loader) in enumerate(zip(train_loaders, valid_loaders)):
        plot_name = f'LogisticRegression_scratch_fold_{fold}'
        ...
        if epoch == epochs - 1 or epoch == last_epoch:
            ...
            with ((torch.no_grad())):
                for samples, labels in valid_loader:
                    ...
                    saliency_map = saliency.attribute(samples, target=None)  # , abs=False)
        ...
        for i in range(len(valid_samples)):
            ...
            ##plt.subplot(1, 2, 1)
            # sample = np.reshape(valid_samples[i][0].cpu().numpy(), (128*384))
            sample = valid_samples[i][0].view(128, 384)
            ...
            # saliency_sample = np.reshape(saliency_maps[i][0], (128*384))
            saliency_sample = saliency_maps[i][0].view(128, 384)
            ##plt.subplot(1, 2, 2)
...
for test_loader in test_loaders:
    with ((torch.no_grad())):
        for i, samples in enumerate(test_loader):
            ...
            saliency_map = saliency.attribute(samples, target=None)
...
for i in range(len(test_samples)):
    ...
    ##plt.figure(figsize=(10, 5))
    ##plt.subplot(1, 2, 1)
    # sample = np.reshape(test_samples[i][0].cpu().numpy(), (128*384))
    sample = test_samples[i][0].view(128, 384)
    print('shape of sample after reshaping:', sample.shape)
    sample = sample.cpu().numpy()
    plt.imshow(sample, cmap='jet')
    plt.title("Original Image")
    ...
    # saliency_sample = np.reshape(saliency_test_maps[i][0], (128*384))
    saliency_sample = saliency_test_maps[i][0].view(128, 384)
    ##plt.subplot(1, 2, 2)
    saliency_sample = saliency_sample.cpu().numpy()
    plt.imshow(saliency_sample, cmap='jet')
    plt.title("Saliency Map")
