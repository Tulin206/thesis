...
from TrainingData_16_Mob_V1_PyTorch import train, validate
# from DataLoad_16_Mob_V1_PyTorch import save_plots, get_data, predict_test_data
from DataLoad_16_Mob_V1_PyTorch import save_plots, TrainDataset, TestDataset, predict_test_data
...
# model = MobileNetV1(ch_in=1, n_classes=1).to(device)
...
def S_6_fold_cv():
    ...
    for fold, (train_index, valid_index) in enumerate(skf.split(x_tensor, y_tensor)):
        ...
        with open(f'/mnt/ceph/tco/TCO-Students/Homes/ISRAT/Result_Plot_PyTorch/accuracy_fold_{fold}.csv', 'w',
                  newline='') as csvfile:
        ...
        with open(f'/mnt/ceph/tco/TCO-Students/Homes/ISRAT/Result_Plot_PyTorch/fold_{fold}.csv', 'w',
                  newline='') as csvfile:
            writer = csv.writer(csvfile)
