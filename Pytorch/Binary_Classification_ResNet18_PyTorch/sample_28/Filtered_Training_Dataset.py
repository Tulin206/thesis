...
def load_training_data(array_of_speak,
    ...
    # Convert indices to integers and set the removed spectra to np.nan
    int_indices_to_remove = np.asarray(indices_to_remove).astype(int)
    new_image[:, int_indices_to_remove] = 0

    # # Set the removed spectra to 0 (white) in the new image
    # new_image.ravel()[indices_to_remove] = np.nan

    # Reshape the filtered image back to the original shape
    filtered_image = np.reshape(new_image, array_of_speak.shape)

    print("Shape of copied image after filtering out the outliers and making them white:", filtered_image.shape)

    # reshape the image to feed into the deep model
    filtered_reshaped_image = filtered_image.reshape(441, 128, 384)
    print("shape of new image for feeding into the ResNet 18:", filtered_reshaped_image.shape)

    training_set_num = np.array(filtered_reshaped_image)
    ...
    # # Plot a small portion of the original and filtered spectra
    # plt.figure(figsize=(10, 5))
    # plt.subplot(2, 1, 1)
    # plt.plot(array_of_speak[0, :400], label='Original Spectrum')
    # plt.title('Original Spectrum')
    # plt.subplot(2, 1, 2)
    # plt.plot(filtered_spectra_data[0, :400], label='Filtered Spectrum')
    ...
    # plt.subplot(1, 2, 1)
    # plt.imshow(np.reshape(np.sum(array_of_speak, axis=0), (128, 384)), cmap="jet")
    # plt.title('Original Image')
    #
    # # Plot the filtered image on the right side
    # plt.subplot(1, 2, 2)
    # plt.imshow(np.reshape(np.sum(filtered_image, axis=0), (128, 384)), cmap="jet")
