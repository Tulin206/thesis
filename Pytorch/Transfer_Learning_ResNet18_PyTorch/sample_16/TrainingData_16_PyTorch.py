...
def train(model, trainloader, optimizer, criterion, device):
    ...
    for i, data in tqdm(enumerate(trainloader), total=len(trainloader)):
        ...
        # # round up model's prediction
        # preds = np.round(predicted_probabilities.detach().numpy())
        # print("print preds after rounding up:", preds)

        from DataLoad_16_PyTorch_TransferLearning import calculate_class_weights

        # Generate true pancreatic texture labels for all 16 samples
...
def validate(model, testloader, criterion, device):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(testloader), total=len(testloader)):
            ...
            # Forward pass.
            outputs = model(image)
            print("raw logits of model's output:", outputs)
            ...
            # Calculate the predicted output
            predicted_probabilities = torch.sigmoid(outputs.cpu())  # Apply sigmoid to get probabilities
            print("print output after applying sigmoid:", predicted_probabilities)

            # # round up model's prediction on validation data
            # preds = np.round(predicted_probabilities.detach().numpy())
            # print("print preds on validation data after rounding up:", preds)
